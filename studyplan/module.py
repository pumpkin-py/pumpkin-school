...
DEGREE_YEARS = {"Bakalářské": 3, "Navazující magisterské": 2, "Doktorské": 4}
...
class StudyPlan(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        try:
            self._load_programmes(self.save_dir / "programmes.csv")
            self._load_subjects(self.save_dir / "subjects.csv")
    ...
    def _load_programmes(self, path):
        ...
        df = pd.read_csv(path, encoding="utf8", sep=";", dtype=dtypes)
    ...
    def _load_subjects(self, path):
        converters = {
            "abbreviation": str,
            "name": str,
            "semester": literal_eval,
            "degree": literal_eval,
            "institute": literal_eval,
            "guarantors": literal_eval,
            "teachers": literal_eval,
            "link": literal_eval,
            "programmes": literal_eval,
        }

        df = pd.read_csv(path, encoding="utf8", sep=";", converters=converters)

        if sorted(list(converters.keys())) == sorted(list(df.columns)):
            self.subjects = df
        else:
            os.remove(path)
            raise ValueError
    ...
    async def load_programmes(self, ctx, attachment: discord.Attachment):
        """Load programmes CSV from a file."""
        save_path = self.save_dir / "programmes.csv"
    ...
    async def load_subjects(self, ctx, attachment: discord.Attachment):
        """Load subjects CSV from a file."""

        save_path = self.save_dir / "subjects.csv"
        await attachment.save(save_path)
    ...
    async def check_subjects(self, ctx, degree: str = None):
        ...
        for subject in dic_subjects:
            if degree not in subject["degree"] and not subject[
                "abbreviation"
            ].startswith("XP"):
                continue
            elif subject["abbreviation"].startswith("XP") and degree != "Bakalářské":
                continue
            ...
            category = discord.utils.get(
                ctx.guild.categories, name=subject["institute"][0]
            )
            if category is None:
                missing_categories.add(subject["institute"][0])
            if channel is None:
                missing_channels.add(subject["abbreviation"])
            elif channel.category.name != subject["institute"][0]:
                channels_in_wrong_categories.append(
                    f"{subject['abbreviation']} ({channel.category.name} -> {subject['institute'][0]})"
                )
    ...
    async def create_and_modify_subjects(self, ctx, degree: str = None):
        """Create all missing rooms for the programmes in the scraped data. Makes sure the title is correct. Also resets the permissions so users need to reassign themselves those rooms."""
        degrees = list(self.programmes["degree"].unique())
        raw_institutes = list(self.subjects["institute"].tolist())
        institutes = list(set([x[0] for x in raw_institutes]))
        ...
        for subject in dic_subjects:
            ...
            if category is None:
                ...
                category = await ctx.guild.create_category_channel(
                    name=subject["institute"][0], overwrites=category_overwrites
                )
            if channel is None:
                ...
                try:
                    ...
                    print(subject["abbreviation"], subject["institute"][0])
                    if "Maximum number of channels in category reached" in str(e):
                        new_category = discord.utils.get(
                            ctx.guild.categories, name=f"{subject['institute'][0]}-2"
                        )
                        if new_category is None:
                            missing_categories.add(f"{subject['institute'][0]}-2")
                        ...
                        missing_channels.add(subject["abbreviation"])
            elif channel.category.name.rstrip("-2") != subject["institute"][0]:
        ...
        await ctx.reply(
            f"```{len(missing_categories)} missing categories created\n{len(missing_channels)} missing channels created\n{len(channels_in_wrong_categories)} channels moved\n{len(channels_wrong_topic)} channels topic edited```"
        )
